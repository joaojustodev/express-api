...
async function registerSessionController (req, res) {
  const { name, email, password } = req.body;
  try {
    if (!name || !email || !password) {
      return res.status(401).json({ error: "All fields is required", fields: "name, email and password" });
    }
...
export default registerSessionController;
